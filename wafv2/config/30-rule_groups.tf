resource "aws_wafv2_rule_group" "ip_lists" {
  name        = "IP_Lists"
  ...
  rule { # 1 WCU
    name     = aws_wafv2_ip_set.static_allowed_ips["IPV4"].name
    priority = 0
    ...
    statement {
      ip_set_reference_statement {
        arn = aws_wafv2_ip_set.static_allowed_ips["IPV4"].arn
      }
    }

    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = aws_wafv2_ip_set.static_allowed_ips["IPV4"].name
      sampled_requests_enabled   = true
    }
  }
  rule { # 1 WCU
    name     = aws_wafv2_ip_set.static_allowed_ips["IPV6"].name
    priority = 1
    ...
    statement {
      ip_set_reference_statement {
        arn = aws_wafv2_ip_set.static_allowed_ips["IPV6"].arn
      }
    }

    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = aws_wafv2_ip_set.static_allowed_ips["IPV6"].name
      sampled_requests_enabled   = true
    }
  }
  rule { # 1 WCU
